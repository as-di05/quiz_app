...
class AuthController {
	...
	async registerAdmin(req, res) {
		try {
			...
			const { rows: arrId } = await db.query(
				'insert into users (login, password, name, surname, patronymic, activ, visible) values ($1, $2, $3, $4, $5, $6, $7) returning id_user',
				[login, hashPassword, name, surname, patronymic, activ, visible]
			)

			res.status(200).json({
				message: 'Вы успешно зарегистрированы как админ',
				type: 'success',
				data: { arrId },
			})
		} catch (e) {
